    ...
    <div className="w-full max-w-md space-y-4">
      ...
      {showNoEmail && (
        <Alert className="bg-red-500/10 border-red-500/50 text-red-400">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            An email address is required to create an account. Please make sure your Discord account
            has an email associated with it and try again.
          </AlertDescription>
        </Alert>
      )}
      {showNotLoggedIn && (
        <Alert className="bg-yellow-500/10 border-yellow-500/50 text-yellow-400">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>You are not logged in. Please log in to continue.</AlertDescription>
        </Alert>
      )}
          ...
          <Button
            onClick={() =>
              signIn.social({
                provider: "discord",
                callbackURL: "/admin",
                errorCallbackURL: "/auth/login?disabled=true",
              })
            }
