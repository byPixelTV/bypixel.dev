          ...
          <>
            ...
            >
              <div className="relative mb-8 sm:mb-12 group">
                ...
                <div className="relative w-36 h-36 sm:w-64 sm:h-64 rounded-4xl sm:rounded-[3rem] overflow-hidden border border-white/10 shadow-2xl">
                  <Image src="/projects/eramc.png" alt="EraMC Icon" fill className="object-cover" />
                </div>
              </div>
                ...
                <div className="absolute bottom-8 left-8 right-8 sm:bottom-16 sm:left-16 sm:right-16 flex flex-col items-start sm:items-end sm:flex-row justify-between gap-6 sm:gap-12">
                  <div className="max-w-2xl">
                    ...
                    <p className="text-base sm:text-xl text-slate-300 leading-relaxed font-medium">
                      Architecture designed for thousands. We don&apos;t just host servers; we
            ...
            >
              <div className="text-center mb-12 sm:mb-24">
                <h3 className="text-4xl sm:text-8xl font-black text-white mb-4 sm:mb-6 tracking-tighter italic">
                  THE MANIFESTO
                </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-10 max-w-7xl w-full">
                {highlights.map((h) => (
                  <motion.div
                    key={h.title}
                    whileHover={
                      isMobile ? {} : { y: -15, backgroundColor: "rgba(255,255,255,0.08)" }
                    }
                  ...
                  >
                    <div
                      className={`w-12 h-12 sm:w-20 sm:h-20 rounded-2xl sm:rounded-3xl bg-linear-to-br ${h.color} flex items-center justify-center mb-6 sm:mb-10 shadow-2xl`}
                    >
                      <Icon icon={h.icon} className="text-2xl sm:text-4xl text-white" />
                    </div>
                    <h4 className="text-xl sm:text-3xl font-bold text-white mb-2 sm:mb-4 italic tracking-tight">
                      {h.title}
                    </h4>
            ...
            >
              <div className="max-w-7xl w-full overflow-y-auto max-h-[90vh] sm:max-h-none sm:overflow-visible pr-2">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-20 items-center">
                  <div className="space-y-6 sm:space-y-12">
                    <div className="space-y-4 sm:space-y-6">
                      <span className="text-purple-400 font-black tracking-[0.4em] uppercase text-[10px] sm:text-sm">
                        Technical Philosophy
                      </span>
                      <h3 className="text-4xl sm:text-6xl font-black text-white italic tracking-tighter uppercase leading-tight">
                        Async-First
                        <br />
                        Architecture
                      </h3>
                      <p className="text-sm sm:text-xl text-slate-300 leading-relaxed">
                        We push the boundaries of what Minecraft servers can handle. By running
                        logic <b>strictly asynchronously</b> on the gameservers, we maintain peak
                        performance while keeping the central database constantly in sync for
                        external API and tooling access.
                      </p>
                      <p className="text-xs sm:text-lg text-slate-400 leading-relaxed border-l-4 border-purple-500 pl-4 sm:pl-6 italic">
                        Complex problems require bespoke solutions. When existing software fails, we
                        build our own ecosystem of external CLIs and monitoring tools.
                      </p>
                    </div>

                    <div className="grid grid-cols-2 gap-4 sm:gap-6">
                      <div className="p-4 sm:p-8 bg-white/5 rounded-3xl sm:rounded-[2.5rem] border border-white/10 group">
                        <span className="text-xl sm:text-4xl font-bold text-white block mb-1 sm:mb-2 tracking-tighter">
                          Dual-Sync
                        </span>
                        <span className="text-[8px] sm:text-xs text-slate-500 uppercase font-bold tracking-widest">
                          Async Logic / DB Persistence
                        </span>
                      </div>
                      <div className="p-4 sm:p-8 bg-white/5 rounded-3xl sm:rounded-[2.5rem] border border-white/10 group">
                        <span className="text-xl sm:text-4xl font-bold text-white block mb-1 sm:mb-2 tracking-tighter">
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-3 gap-3 sm:gap-5">
                    {techStack.map((tech) => (
                      ...
                      >
                        ...
                        <span className="text-[8px] sm:text-xs font-black text-white text-center uppercase tracking-widest">
                          {tech.name}
                        </span>
                        <span className="text-[6px] sm:text-[8px] text-purple-400 uppercase font-black mt-1 sm:mt-2 tracking-tighter">
                          {tech.category}
                        </span>
                      </motion.div>
                    ))}
                ...
                <motion.div className="mt-10 sm:mt-20 p-6 sm:p-12 bg-linear-to-r from-purple-900/30 to-blue-900/20 border border-white/10 rounded-4xl sm:rounded-[3.5rem] backdrop-blur-3xl text-center relative overflow-hidden">
                  ...
                  <p className="text-sm sm:text-2xl text-slate-200 leading-relaxed italic max-w-5xl mx-auto">
                    &quot;We don&apos;t just write plugins; we architect a global mesh. Every
                    gameserver operates as a high-speed async node, feeding a centralized data layer
                    that empowers our entire web and CLI tooling infrastructure.&quot;
                  </p>
                </motion.div>
              </div>
            </motion.div>
