...
const fixtures = [
  { _id: new ObjectId(), userId: authorId, slug: "first", title: "First", content: "LARGE ARTICLE BODY", draft: false },
  { _id: new ObjectId(), userId: authorId, slug: "second", title: "Second", content: "ANOTHER ARTICLE BODY", draft: false },
  { _id: new ObjectId(), userId: authorId, slug: "private", title: "Private draft", content: "PRIVATE BODY", draft: true },
];
let admin = false;
let authorQueries = 0;
let listOptions;
...
mock.module("@/lib/mongo", () => ({
  getPostsCollection: async () => ({
    find: (filter, options) => {
      listOptions = options;
      return { sort: () => ({ toArray: async () => fixtures.filter(post => filter.draft === undefined || post.draft === filter.draft).map(post => {
        const result = { ...post };
        if (options?.projection?.content === 0) delete result.content;
        return result;
      }) }) };
    },
    findOne: async filter => fixtures.find(post => post.slug === filter.slug && (filter.draft === undefined || post.draft === filter.draft)) ?? null,
  }),
...
beforeEach(() => { admin = false; authorQueries = 0; listOptions = undefined; });
...
describe("Blog read performance and privacy", () => {
  ...
  test("requesting draft previews does not grant a visitor draft access", async () => {
    ...
    expect((await getPostBySlug("first", { includeDraftsForAdmin: true })).post?.slug).toBe("first");
  });
  test("admins can preview drafts only when explicitly requested", async () => {
    admin = true;
    expect((await getPostBySlug("private")).post).toBeNull();
    expect((await getPostBySlug("private", { includeDraftsForAdmin: true })).post?.draft).toBe(true);
  });
});
