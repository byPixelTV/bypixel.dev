...
const {
  chromium,
} = require("C:/Users/Pixel/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules/playwright");
const executablePath = path.join(
  process.env.TEMP,
  "bypixel-browser-check/chromium-1234/chrome-win64/chrome.exe",
);
(async () => {
  ...
  for (const route of routes) {
    ...
    if (route === "/") {
      data.scroll = await page.evaluate(
        () =>
          new Promise((resolve) => {
            const frames = [];
            ...
            const step = (now) => {
              ...
              if (progress < 1) requestAnimationFrame(step);
                ...
                resolve({
                  ...
                  p95: [...frames].sort((a, b) => a - b)[Math.floor(frames.length * 0.95)],
                  max: Math.max(...frames),
                  over25: dropped.length,
                });
              }
            };
